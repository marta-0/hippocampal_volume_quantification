...
Total Volume: {inference['total']}""",
    ...
    pil_i = Image.fromarray(nd_img, mode="L").convert("RGB").resize((400,400))
    ...
    pred_vol = pred_vol[:orig_vol.shape[0], :orig_vol.shape[1], :orig_vol.shape[2]] 
    ...
    contour_1 = pil_pred.point(lambda p:p==127)
    edges = contour_1.filter(ImageFilter.FIND_EDGES)
    edges = np.array(edges)
    output[np.nonzero(edges)] = (255,0,0)
#     output_pred[np.nonzero(edges)] = (255,0,0)
    
    contour_2 = pil_pred.point(lambda p:p==255)
    edges = contour_2.filter(ImageFilter.FIND_EDGES)
    edges = np.array(edges)
    output[np.nonzero(edges)] = (0,255,0)    
#     output_pred[np.nonzero(edges)] = (0,255,0)
    
    output = Image.fromarray(output, mode="RGB")#.resize((400,400))
    pimg.paste(output, box=(810, 280))
    
    output_pred = Image.fromarray(output_pred, mode="RGB")#.resize((400,400))
    pimg.paste(output_pred, box=(410, 280))
